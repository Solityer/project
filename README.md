...
- `project` 同级目录中的 `GAT-for-PPI`
  只作为上游真实参数来源与原始训练参考，不是当前 formal / benchmark 运行主线。
...
- benchmark 主表只统计：
  ...
  - `verify_time_ms`
  - `proof_size_bytes`
- 训练时间、训练日志、训练超参数调优都不进入 formal benchmark 主表。
...
### 4. PPI：上游训练、导入、formal prove/verify

PPI 训练不是当前 project 的核心目标。推荐把同级目录中的原始 `GAT-for-PPI` 当作上游参数来源，在那里训练出真实 checkpoint，再通过当前 project 的唯一 importer 导入。

当前唯一正式主线是：

上游真实训练 checkpoint
-> `scripts/import_ppi_bundle.py`
-> `artifacts/checkpoints/ppi_gat`
-> `configs/ppi_batch_formal.cfg`
-> 当前 project prove/verify
-> 当前 project benchmark 总表

如果上游训练产物是 TensorFlow checkpoint 前缀：

```bash
./.venv/bin/python scripts/import_ppi_bundle.py --tf-checkpoint-prefix <上游_checkpoint_prefix> --output-dir artifacts/checkpoints/ppi_gat
```

如果上游训练产物已经是 torch checkpoint：

```bash
./.venv/bin/python scripts/import_ppi_bundle.py --torch-checkpoint <上游_best_model.pt> --output-dir artifacts/checkpoints/ppi_gat
```

导入后，运行全量 295 图的正式 prove/verify：
...
- 同级目录中的原始 `GAT-for-PPI` 只承担上游真实参数来源职责，不承担当前 project 的 formal / benchmark 运行职责。
